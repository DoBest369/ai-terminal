...
- [~] **S9 SFTP 双端深化 + 细节打磨**（进行中）：✅ apple SFTP 标杆确认（Citadel 真 SFTP 全操作）；✅ windows/linux SFTP 下载（base64）+ 删除（嵌套确认）双端对齐，能力一致（浏览/导航/预览/下载/删除）；✅ windows AI 气泡时间戳；✅ windows 命令历史持久化。✅ batch 批量群发移植 windows/linux（多连接并发执行聚合，护城河能力一致）。待办：连接编辑；SFTP 上传/mkdir；主题切换实际生效（U3）。
