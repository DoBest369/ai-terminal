...
- [~] **S9 SFTP 双端深化 + 细节打磨**（进行中）：✅ apple SFTP 标杆确认（Citadel 真 SFTP 全操作）；✅ windows/linux SFTP 下载（base64）+ 删除（嵌套确认）双端对齐，能力一致（浏览/导航/预览/下载/删除）；✅ windows AI 气泡时间戳；✅ windows 命令历史持久化。✅ batch 批量群发移植 windows/linux（多连接并发执行聚合，护城河能力一致）；✅ SFTP 文件操作全覆盖双端（浏览/导航/预览/下载/上传/删除/新建目录/重命名，与 apple 功能对等）；✅ 终端 ANSI 彩色双端。S9 SFTP 深化完成。剩余打磨项：连接编辑；主题切换实际生效（U3）；android/iOS SFTP 增强。
...
- [x] **连接快速复制连接串双端** ✅（apple contextMenu copyConnectionString/android ServerCard DropdownMenu；ssh user@host -p port 默认 22 省 -p；修 android Context 全限定；apple 8 自测+android clean 零 warning；apple 6d6c1ca/android 4693fef→3886b63）
- [x] **质量收口·近期进展快照（PARITY 97 项）** ✅（apple 8 自测+android clean 零 warning；PARITY 97 项 🟡=0；知识卡片增强/导入导出对称/多对话完善 梳理；推送见下）
...
- [x] **质量收口·快捷命令全功能+PARITY 96 项快照** ✅（apple 8 自测+android clean 零 warning；PARITY 96 项 ✅✅ 🟡=0；快捷命令增删改分组收藏导入导出全功能；推送见下）
...
- [x] **排障工作流 Z4 扩充双端** ✅（8→11：加 定时任务排查/日志异常扫描/防火墙规则检查；只读诊断命令；diag 自测内置数=11；双端 builtins 对齐；apple 60c14ba/android 9969b10）
...
- [x] **批量群发结果导出双端** ✅（结果拼 Markdown[命令+统计+各机ok/fail+输出]；apple 复制[edc7648]/android 分享 Intent[06d8cf2]；运维记录留存）
- [x] **质量收口·运维数据维度全景快照** ✅（apple 8 自测+android clean 零 warning；PARITY 🟡=0；CPU/内存/磁盘/负载/运行时长/服务 6 维度双端齐采集→展示/告警/AI；推送见下）
...
- [x] **质量收口·批量运维完整度快照** ✅（apple 8 自测+android clean 零 warning+APK；PARITY 🟡=0；群发[全选/分组选/统计/AI汇总]+巡检[告警置顶/AI总结/负载运行时长]双端完整；推送见下）
- [x] **批量群发结果成功/失败统计双端** ✅（审计双端群发结果无汇总统计→双端同步加「✅成功N·❌失败M·共K台」统计行；batch 自测过；apple a8d1240/android ef5be14）
- [x] **docs/PRODUCT.md 微更新** ✅（SFTP 批量删/下载、状态面板负载/运行时长、快捷命令增删改分组、导入去重反馈、色选器校验、AI 提示词25+快捷追问、知识闭环筛选检索；apple build 抽查；推送 6ae4cff）
- [x] **质量收口·运维数据链贯穿快照** ✅（apple 8 自测+android clean 零 warning；PARITY 🟡=0；采集→面板→巡检→健康→沉淀 数据链贯穿；推送见下）
...
- [x] **android 状态面板加负载/运行时长** ✅（审计发现 apple 早有 loadavg/uptime，android 缺→补 fetchStatus uptime+ServerStatus load/uptime+面板第二行；状态面板双端对齐；apple 8 自测+android clean 零 warning；推送 d83273f）
...
- [x] **终端区功能完整度评估** ✅（逐项审计：控制键栏/输出搜索/复制全部/清屏/字号/自动滚动/ANSI/快捷命令历史补全/风险徽章 全双端✅✅，无缺口；如实记录已齐，不强行加功能；推送见下）
- [x] **质量收口·AI 辅助能力快照** ✅（apple 8 自测+android clean 零 warning；PARITY 🟡=0；AI 从解释/对话到结合本机历史给结论到真执行+沉淀 全链路双端齐；推送见下）
- [x] **AI 运维提示词库扩充双端** ✅（5 类×3→5 条；加 数据库排查/端口定位/Nginx反代/systemd自启/防火墙/进程CPU/磁盘IO/日志归类 等；apple 1e482ff/android 4aa22c7）
- [x] **apple ConnectionEditShowcase 补色选器截图** ✅（Showcase 加颜色标签卡片+必填标识；渲染 04-connection-edit.png 核对+拷 screenshots；截图与功能同步；8 自测全过；推送 b065187）
...
- [x] **危险操作二次确认审计对齐** ✅（审计双端破坏性操作确认；android 补 删除连接/删除对话 二次确认[d231188]；SFTP/批量/清空 双端本有；删除卡片/快捷命令属低破坏性单条 swipe/icon 可接受；apple 8 自测全过）
- [x] **android AI 清空对话二次确认** ✅（对齐 apple 早有 confirmationDialog；防误触丢对话；apple 8 自测+android clean 零 warning；推送 5c3905a）
