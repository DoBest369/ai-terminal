...
> 🛡️ **2026-06-28 全功能成熟度（windows/linux 双端对齐 apple，69 阶段/1020 提交）**：
> - **安全**：命令风险四级实时提示（输入即警示）+ AI 三模式危险命令确认（执行需确认，Auto 也守）+ SFTP 写操作确认 + 凭据走环境变量不硬编码。
> - **AI 能力**：三模式（Chat/Agent/Auto 自主闭环）+ 多会话（独立上下文+持久化）+ 搜索 + 导出（Markdown）+ 代码块点击插入命令 + 用户消息重发 + 护城河 Z1-Z3 一键真闭环。
> - **监控套件六维**：状态条聚合（CPU/内存/磁盘/负载+30s刷新+>90%告警+服务状态）→ 系统/进程/端口/磁盘/登录/防火墙下钻（整合为监控下拉+tab+刷新）。
> - **连接管理**：CRUD + 分组（输入/持久化/编辑）+ 搜索 + 可达探测 + 导入导出（json 批量）。
> - **终端**：ANSI 彩色 + 搜索高亮 + 字号可调+持久化 + 导出 + 清屏/滚到底 + 六维监控工具。
> - **命令**：历史面板 + ↑↓ 回溯 + 快捷命令/追问自定义（增删+持久化）+ batch 群发 + 风险提示。
