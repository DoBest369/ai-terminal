...
| 复制 ssh 连接串 | ✅ | ✅ | 双端复制 ssh user@host -p port（默认 22 省 -p），粘贴到其他终端/文档 |
...
| 对话内搜索 | ✅ | ✅ | 当前对话消息按关键词过滤 |
| AI 对话全局搜索 | ✅ | ✅ | 搜索激活时对话切换器标注各对话匹配条数（🔍N），跨对话定位历史内容 |
| 多行输入 | ✅ | ✅ | 粘贴报错/长指令 |
...
| 知识卡片导入 | ✅ | ✅ | 双端粘贴解析 Markdown（## 类型 + - 内容）+去重，对齐导出 |
| 知识卡片随手记 | ✅ | ✅ | 双端命令历史一键存为知识卡片 |
| 知识卡片类型筛选 | ✅ | ✅ | 双端按 全部/问题/方案/笔记 筛选 |
| 知识卡片自由标签 | ✅ | ✅ | 双端录入逗号分隔标签 + 卡片显 #标签（持久化向后兼容） |
| 知识卡片标签筛选 | ✅ | ✅ | 双端按 #标签 Chip 筛选（与 类型筛选+关键词搜索 组合） |
| 知识卡片关键词搜索 | ✅ | ✅ | 双端类型筛选+关键词组合过滤（apple .searchable / android 搜索框） |
| AI 结论一键存为方案 | ✅ | ✅ | 双端 AI 回复后一键存为方案卡片（发现问题→AI分析→沉淀方案闭环） |
| 排障结论存为方案 | ✅ | ✅ | 排障 AI 结论一键存方案（apple AI对话区存方案入口 / android 终端区「存为方案」Chip） |
