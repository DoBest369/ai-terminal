...
struct TermindApp {
    ...
    ai_msgs: Vec<(bool, String)>,  // AI 对话（true=用户提问 / false=AI 真实回复）；当前活跃会话
    sessions: Vec<Vec<(bool, String)>>,  // 多会话存档（对照 windows/apple ai-conv），切换时与 ai_msgs 同步
    ...
    pending_resend: bool,                      // 用户气泡重发触发（下帧发送）
...
/// 加载持久化的自定义快捷命令（对照 windows customCmds）
fn load_custom_cmds() -> Vec<String> {
    config_path()
        .and_then(|p| std::fs::read_to_string(&p).ok())
        .and_then(|t| serde_json::from_str::<serde_json::Value>(&t).ok())
        .and_then(|j| j["custom_cmds"].as_array().map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect()))
        .unwrap_or_default()
}
...
impl eframe::App for TermindApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok((cpu, mem, disk, load, svcs)) = self.metrics_rx.try_recv() {
            self.metrics = (cpu, mem, disk, load);
            self.services = svcs;
        }
        ...
        if active_host != self.metrics_target || now - self.last_refresh > 30.0 {
            ...
            if host_changed { self.metrics = (0, 0, 0, "…".to_string()); self.services.clear(); }
            ...
            std::thread::spawn(move || {
                ...
                // 一条命令取齐：负载 + 内存 + 磁盘(df 根分区) + /proc/stat 两次采样算 CPU% + 关键服务 systemctl
                ...
                let disk_pct: u8 = l[2].trim().parse().unwrap_or(0);
                let c1: Vec<f64> = l[3].split_whitespace().filter_map(|s| s.parse().ok()).collect();
                let c2: Vec<f64> = l[4].split_whitespace().filter_map(|s| s.parse().ok()).collect();
                ...
                let _ = tx.send((cpu_pct, mem_pct, disk_pct, load, svcs));
            });
        }
        ...
        egui::SidePanel::right("ai")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    // 会话管理 / 清空 / 导出对话（对照 windows/apple ai-conv）
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        // 多会话切换/新建/删除（对照 windows）
                        ui.menu_button(egui::RichText::new(egui_phosphor::regular::CHATS).size(13.0).color(TEXT_SECONDARY()), |ui| {
                            ui.set_max_width(260.0);
                            ui.label(egui::RichText::new("AI 会话").size(12.0).color(TEXT_PRIMARY()));
                            for i in 0..self.sessions.len() {
                                let msgs = if i == self.cur_session { &self.ai_msgs } else { &self.sessions[i] };
                                let first = msgs.iter().find(|(u, _)| *u).map(|(_, t)| t.clone()).unwrap_or_default();
                                let title = if first.is_empty() { format!("新会话 {}", i + 1) } else if first.chars().count() > 18 { format!("{}…", first.chars().take(18).collect::<String>()) } else { first };
                                let cur = i == self.cur_session;
                                ui.horizontal(|ui| {
                                    if ui.add(egui::Button::new(egui::RichText::new(format!("{}{}", if cur { "● " } else { "" }, title)).size(11.0).color(if cur { ACCENT() } else { TEXT_PRIMARY() })).frame(false)).clicked() {
                                        session_action = Some(i); ui.close_menu();
                                    }
                                    if self.sessions.len() > 1 && ui.add(egui::Button::new(egui::RichText::new(egui_phosphor::regular::X).size(10.0).color(WARNING())).frame(false)).clicked() {
                                        del_session = Some(i); ui.close_menu();
                                    }
                                });
                            }
                            if ui.button(egui::RichText::new("+ 新建会话").size(11.0).color(SUCCESS())).clicked() {
                                session_action = Some(usize::MAX); ui.close_menu();
                            }
                        });
                    ...
                    // 会话切换/新建：先把当前 ai_msgs 存回 sessions[cur]，再加载目标
                    if let Some(i) = session_action {
                        self.sessions[self.cur_session] = self.ai_msgs.clone();
                        if i == usize::MAX {
                            self.sessions.push(Vec::new());
                            self.cur_session = self.sessions.len() - 1;
                            self.ai_msgs.clear();
                        } else {
                            self.cur_session = i;
                ...
                let mut resend_pick: Option<String> = None;   // 用户气泡重发同问题
                for (is_user, text) in &self.ai_msgs {
                    ui.add_space(6.0);
                    ...
                    if *is_user {
                        ui.horizontal(|ui| {
                            ui.colored_label(TEXT_SECONDARY(), egui::RichText::new("你").size(10.0).strong());
                            // 重新发送同问题（AI 交互增强，对照 windows）
                            if ui.add(egui::Button::new(egui::RichText::new(egui_phosphor::regular::ARROW_CLOCKWISE).size(10.0).color(TEXT_SECONDARY())).frame(false)).on_hover_text("重新发送").clicked() {
                                resend_pick = Some(text.clone());
                            }
                        });
                ...
                ui.horizontal(|ui| {
                    ...
                    if (send.clicked() || enter || std::mem::take(&mut self.pending_resend)) && !self.ai_input.trim().is_empty() && !self.ai_busy {
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    // CPU/内存/磁盘 mini 进度条：远程选中服务器真实指标（SSH 取 /proc + df，对照 windows）
                    let (cpu_pct, mem_pct, disk_pct, load) = (self.metrics.0, self.metrics.1, self.metrics.2, self.metrics.3.clone());
                    ...
                    ui.colored_label(TEXT_SECONDARY(), "磁盘");
                    ui.add(egui::ProgressBar::new(disk_pct as f32 / 100.0).desired_width(54.0)
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        // 监听端口（SSH ss 取监听端口+进程 → 终端展示，深化监控，对照 windows）
                        if ui.add(egui::Button::new(egui::RichText::new(egui_phosphor::regular::PLUGS_CONNECTED).size(14.0).color(TEXT_SECONDARY())).frame(false))
                            .on_hover_text("监听端口（网络服务）").clicked() {
                            self.term_lines.push("# 监听端口（TCP）…".to_string());
                            let (host, user, tx) = (active_host.clone(), active_user.clone(), self.term_tx.clone());
                            std::thread::spawn(move || {
                                let pass = std::env::var("TERMIND_SSH_PASS").unwrap_or_default();
                                if pass.is_empty() { let _ = tx.send("⚠️ 未配置 SSH 密码".to_string()); return; }
                                let r = ssh_exec(&host, 22, &user, &pass, "ss -tlnp 2>/dev/null | head -20");
                                let _ = tx.send(r);
                            });
                        }
                        ...
                        // 登录用户（SSH who+last → 终端展示，安全运维，对照 windows）
                        if ui.add(egui::Button::new(egui::RichText::new(egui_phosphor::regular::USERS).size(14.0).color(TEXT_SECONDARY())).frame(false))
                            .on_hover_text("登录用户（在线 + 最近登录）").clicked() {
                            self.term_lines.push("# 登录用户（who + last）…".to_string());
                            let (host, user, tx) = (active_host.clone(), active_user.clone(), self.term_tx.clone());
                            std::thread::spawn(move || {
                                let pass = std::env::var("TERMIND_SSH_PASS").unwrap_or_default();
                                if pass.is_empty() { let _ = tx.send("⚠️ 未配置 SSH 密码".to_string()); return; }
                                let r = ssh_exec(&host, 22, &user, &pass, "echo '== 在线 =='; who; echo '== 最近 =='; last -n 8 2>/dev/null | head -8");
                                let _ = tx.send(r);
                            });
                        }
                ...
                egui::ScrollArea::horizontal().max_height(28.0).show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        // 自定义快捷命令（绿，点击填入 / 右键删除，对照 windows）
                        let mut del_cmd: Option<String> = None;
                        for cmd in &self.custom_cmds {
                            let resp = ui.add(egui::Button::new(
                                egui::RichText::new(cmd).monospace().size(11.0).color(SUCCESS()))
                                .fill(SUCCESS().linear_multiply(0.12)).rounding(14.0));
                            if resp.clicked() { self.cmd_input = cmd.clone(); }
                            resp.context_menu(|ui| { if ui.button(format!("删除「{}」", cmd)).clicked() { del_cmd = Some(cmd.clone()); ui.close_menu(); } });
                        }
                        ...
                        ui.menu_button(egui::RichText::new("+").size(14.0).color(TEXT_SECONDARY()), |ui| {
                            ui.set_max_width(240.0);
                            ui.label(egui::RichText::new("添加快捷命令").size(12.0).color(TEXT_PRIMARY()));
                            ui.add(egui::TextEdit::singleline(&mut self.new_cmd_input).hint_text("自定义命令…").font(egui::TextStyle::Monospace));
                            if ui.button("添加").clicked() {
                                let c = self.new_cmd_input.trim().to_string();
